...
(defun construct-random-transformation (state-vector)
  (let (case (random 4)); this is the major case definition
    (cond ((equal case 0) (OUSwitch state-vector)); Otonality/Utonality switch
          ((equal case 1) (octave state-vector)); go up or down an octave
          ((equal case 2) (nexus state-vector)); pick a new Numerary Nexus
          ((equal case 3) (idents state-vector)); add or drop an identity
          (t state-vector))))

(defun OUSwitch (state-vector)
"Flip Otonality/Utonality switch"
  (cons (* -1 (first state-vector)) (rest state-vector)))
